...
def filter_top_k_answers(answers_dset, top_k=1000):
    ...
    freq = []
    for answer in occurence.keys():
        freq.append({'answer': answer, 'frequency': len(occurence[answer])})

    freq = sorted(freq, key=lambda x: x['frequency'], reverse=True)
    freq = freq[0:top_k]
    freq_answers = {}
    for f in freq:
        freq_answers[f['answer']] = f['frequency']

    filt_occurence = {}
    occ_keys = occurence.keys()
    for answer in occ_keys:
        if answer in freq_answers:
            #occurence.pop(answer)

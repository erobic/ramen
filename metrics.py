...
class Metrics:
    ...
    def update_per_batch(self, model, answers, loss, pred, visual_features):
        ...
        self.loss += loss.data * visual_features.size(0)
        self.raw_score += batch_score
        curr_size = visual_features.shape[0]
        self.num_examples += curr_size

    def update_per_epoch(self):
        self.loss /= self.num_examples
        self.normalized_score = 100 * self.raw_score / self.upper_bound
        self.score = 100 * self.raw_score / self.num_examples
...
def accumulate_metrics(epoch, train_metrics, val_metrics, val_per_type_metric, is_best,
                       best_val_score,
    ...
    stats = {
        ...
        "train_raw_score": float(train_metrics.raw_score / train_metrics.num_examples),
        "train_normalized_score": float(train_metrics.normalized_score),
        "train_upper_bound": float(train_metrics.upper_bound / train_metrics.num_examples),
    ...
    if save_val_metrics:
        ...
        stats["val_upper_bound"] = float(val_metrics.upper_bound / val_metrics.num_examples)
